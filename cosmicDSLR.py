#cosmicDSLR: a program for searching for cosmic ray traces in digital camera images
...
def process(rawdir, fileLim, batchMode):
    ...
    for i,f in enumerate(filelist[:fileLim]):
        if f not in doneFileList:
            ...
            print("File "+f+" already loaded. Skipping.")
    
    sigmaLim = np.sqrt(runNum) + 1.0
    bbox = 10
    sbox = range(-bbox,bbox+1)
    marker = list(range(-bbox,bbox+1)[:5])+list(range(-bbox,bbox+1)[-5:])
...
## Define the command line input
    ...
    while True:
        rawdir = args[0]    #possibly make this handle a list of input directories
